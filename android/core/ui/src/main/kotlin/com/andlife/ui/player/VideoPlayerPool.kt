...
//    private var cacheDataSourceFactory: CacheDataSource.Factory? = null
    ...
    private val activePlayers = LinkedHashMap<String, VideoPlayer>(
        MAX_POOL_SIZE,
        0.75f,
        true
    ) // accessOrder=true -> 최근 접근한 순서대로 정렬, LRU 관리를 하려면 LinkedHashMap이 좋다.
    ...
    fun preparePlayers(context: Context) {
        ...
        repeat(MAX_POOL_SIZE) {
            ...
            val videoPlayer = VideoPlayer(exoPlayer, uri = "")
            playerInstances.add(videoPlayer)
        }
    }
    ...
    fun getPlayer(uri: String): VideoPlayer {
        ...
        val playerToUse = if (activePlayers.size < playerInstances.size) {
            ...
            val oldestUri = activePlayers.keys.first()
            val removedPlayer = activePlayers.remove(oldestUri)!! // 가장 오래된 플레이어 제거
            removedPlayer.stop() // 상태 패턴에 의해 내부 exoPlayer.stop() 및 상태 변경 발생
            removedPlayer // 재사용을 위해 반환
        }

        //playerToUse.uri = uri
        val updatedPlayer = VideoPlayer(playerToUse.exoPlayer, uri)
        val mediaSource = ProgressiveMediaSource
            .Factory(cacheDataSourceFactory!!)
            .createMediaSource(MediaItem.fromUri(uri))

//        activePlayers[uri] = playerToUse
//        return playerToUse
